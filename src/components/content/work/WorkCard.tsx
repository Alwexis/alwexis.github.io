...
export default function WorkCard({ work, extended = false }: WorkCardProps) {
  return (
    <div className="corner-border px-8 py-8 h-fit w-full flex flex-col animate-fade">
      <h3
        className={`text-xl md:text-3xl font-semibold ${spaceGrotesk.className} text-neutral-100`}
      >
        {work.name}
      </h3>
      ...
      <section className="flex items-center justify-start space-x-2 md:space-x-6 my-2 select-none">
        <a href={work.repository} title="Github Repository" target="_blank" rel="noreferrer"
          className={`inline-block px-2.5 py-1 text-sm md:text-xl md:px-4 md:py-2 border border-neutral-300 my-0.5 transition-all capitalize ${geistSans.className} hover:bg-neutral-900`}>
