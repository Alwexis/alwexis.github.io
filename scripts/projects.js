...
let __INITIALITALIZED__ = false;

function saveData(dataToSave) {
  //console.log('Saving data to local storage...');
  if (localStorage.getItem('repositories')) {
    const lastUpdate = JSON.parse(localStorage.getItem('repositories')).lastUpdate;
    const lastUpdateParsed = new Date(lastUpdate);
    if (Math.abs(lastUpdateParsed - Date.now()) > parseMiliseconds(1, 'day')) {
      const rawRepositories = { repositories: dataToSave, lastUpdate: Date.now() };
      const repositoriesJson = JSON.stringify(rawRepositories);
      localStorage.setItem('repositories', repositoriesJson);
    }
  } else {
    const rawRepositories = { repositories: dataToSave, lastUpdate: Date.now() };
    const repositoriesJson = JSON.stringify(rawRepositories);
...
function parseMiliseconds(amount, conversion) {
  switch (conversion) {
    case 'second':
      return amount * 1000;
    case 'minute':
    ...
    case 'month':
      return amount * 1000 * 60 * 60 * 24 * 7 * 4;
    case 'year':
      return amount * 1000 * 60 * 60 * 24 * 7 * 4 * 12;
  }
}
...
const project_Swiper = new Swiper('.project-container', {
  ...
  on: {
    reachBeginning: function (e) {
      ...
      //* Body
      e.slides[e.activeIndex].querySelector('.project-body')
    ...
    slideChange: function (e) {
      ...
      e.slides[e.activeIndex].querySelector('.project-body')
        .style = `animation-name: appearFromBottom; animation-duration: 0.5s; animation-fill-mode: forwards; animation-timing-function: ease-in-out;`
      e.slides[e.previousIndex].querySelector('.project-body')
        .style = `opacity: 0; animation-name: none;`
      //* Footer
      delay = 0;
      e.slides[e.activeIndex].querySelectorAll('.footer-item').forEach((element) => {
        element.style = `animation-name: appearFromLeft; animation-duration: 0.5s; animation-delay: ${delay}s; animation-fill-mode: forwards; animation-timing-function: ease-in-out;`
...
async function init() {
  let shouldLocalInit = false;
  // Data
  const savedData = loadData();
  if (savedData && Math.abs(savedData.lastUpdate - Date.now()) < parseMiliseconds(1, 'day')) {
    shouldLocalInit = true;
  }
  if (shouldLocalInit) {
    localInit();
  } else {
    ...
    //console.log('There are:', data.length, ' projects');
    //let i = 0;
    //for await (let repo of data) {
    await data.forEach(async (repo, index) => {
      //* Template Header
...
function localInit() {
  let projects = document.querySelector('.swiper.project-container .swiper-wrapper');
  const savedData = loadData();
  const data = savedData.repositories;
  data.forEach((repo) => {
    let project = document.createElement('div');
    project.classList.add('project');
    project.classList.add('swiper-slide');

    let header = createHeader(repo);
    //let body = repository.template;
    let body = document.createElement('section');
    body.classList.add('project-body');
    if (repo.template === null || repo.template === undefined || repo.template === '') {
      body.innerHTML = `
        <p class="no-template-error">
          <img src="./assets/media/images/oh oh.png">
          Uh oh... There is no template for this project yet.
      ...
      body.innerHTML = repo.template;
    }
    let footer = createFooter(repo);
